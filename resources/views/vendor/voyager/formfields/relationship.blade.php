	...
	@if(class_exists($options->model))
		...
		@if($options->type == 'belongsTo')
			...
			@if(isset($view) && ($view == 'browse' || $view == 'read'))
				...
				@if(isset($query))
					<p><a href="{{route('voyager.' . str_replace('_', '-', $query->getTable()) . '.show', ['id' => $query->id])}}">{!! $query->toHtml() !!}</a></p>
				@else
					<p>-</p>
				@endif
			...
			@else
				...
				<select class="form-control select2" name="{{ $options->column }}">
					...
					@foreach($query as $relationshipData)
						<option value="{{ $relationshipData->{$options->key} }}" @if($dataTypeContent->{$options->column} == $relationshipData->{$options->key}){{ 'selected="selected"' }}@endif>{!! $relationshipData->toHtml() !!}</option>
					@endforeach
				</select>
		...
		@elseif($options->type == 'hasMany')
			...
			@if(isset($view) && ($view == 'browse' || $view == 'read'))
				...
				@php
					$relationshipData = (isset($data)) ? $data : $dataTypeContent;
					$model = app($options->model);
					$query = $model::where($options->column, '=', $relationshipData->id)->get();

					$selected_values = [];
					foreach($query as $q) {
            			$selected_values[] = '<a href="' . route('voyager.' . str_replace('_', '-', $q->getTable()) . '.show', ['id' => $q->id]) . '">' . $q->toHtml() . '</a>';
					}
				@endphp
				...
				@if(isset($query))
					<ul>
						@foreach($query as $query_res)
							<li><a href="{{route('voyager.' . str_replace('_', '-', $query_res->getTable()) . '.show', ['id' => $query_res->id])}}">{!! $query_res->toHtml() !!}</a></li>
						@endforeach
					</ul>
			...
			@else
				...
				>
					...
					@foreach($relationshipOptions as $relationshipOption)
						<option value="{{ $relationshipOption->{$options->key} }}" @if(in_array($relationshipOption->{$options->key}, $selected_values)){{ 'selected="selected"' }}@endif>{!! $relationshipOption->toHtml() !!}}</option>
					@endforeach
